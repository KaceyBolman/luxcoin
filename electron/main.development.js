...
import { luxLogger } from './lib/remoteLog';
...
try {
  ...
  ipcMain.on('log-to-remote', (event, logEntry) => {
    if (sendLogsToRemoteServer) luxLogger.info(logEntry);
  });
} catch (error) {
...
Log.info(`========== Lux is starting at ${new Date()} ==========`);
Log.info(`!!! Lux is running on ${os.platform()} version ${os.release()}
...
const luxVersion = process.env.LUX_VERSION || 'dev';
...
app.on('ready', async () => {
  ...
  aboutWindow.setTitle('About Lux'); // default title
  ...
  mainWindow.setTitle(`Lux (${luxVersion})`);
