...
usage() {
    ...
    cat >&2 <<EOF
  Usage:
    $0 LUX-VERSION CARDANO-BRANCH OPTIONS*
...
cardano_branch="$(printf '%s' "$1" | tr '/' '-')"; arg2nz "Cardano SL branch to build Lux with" $1; shift
...
CARDANO_ARTIFACT=cardano-binaries     # ex- lux-bridge
...
test -d node_modules/lux-client-api/ -a -n "${fast_impure}" || {
        ...
        tar xJf ${CARDANO_ARTIFACT_FULL_NAME}.tar.xz --strip-components=1 -C node_modules/lux-client-api/
        rm      ${CARDANO_ARTIFACT_FULL_NAME}.tar.xz
        echo "cardano-sl build id is $(cat node_modules/lux-client-api/build-id)"
        if [ -f node_modules/lux-client-api/commit-id ]; then echo "cardano-sl revision is $(cat node_modules/lux-client-api/commit-id)"; fi
        if [ -f node_modules/lux-client-api/ci-url ]; then echo "cardano-sl ci-url is $(cat node_modules/lux-client-api/ci-url)"; fi
        ...
        rm -f node_modules/lux-client-api/cardano-*
}
